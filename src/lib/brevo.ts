...
export async function sendSettingsChangeOtpEmail({ to, action, otp }: { to: string, action: string, otp: string }): Promise<void> {
  const digits = otp.split("").join("</span><span style=\"display:inline-block;width:48px;height:56px;line-height:56px;text-align:center;font-size:28px;font-weight:700;background:#1a1c23;border:1.5px solid rgba(255,255,255,0.3);border-radius:10px;color:#d4d4d8;margin:0 4px;font-family:monospace\">");
  const subject = `Verify Your Action - Founders Hook`;
  const actionText = action === "email" ? "change your email address" : action === "username" ? "change your username" : "change your password";
  const textContent = `You requested to ${actionText}. Your verification code is: ${otp}\n\nThis code expires in 15 minutes.\n\nIf you did not request this, please ignore this email.`;
    ...
    <tr>
      <td align="center" style="padding:48px 16px">
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#13141a;border-radius:20px;border:1px solid rgba(255,255,255,0.08);overflow:hidden">
          ...
          <tr>
            <td style="padding:8px 40px 40px">
              <h1 style="font-size:22px;font-weight:700;color:#fff;margin:0 0 8px;text-align:center">Verify Your Request</h1>
              <p style="font-size:14px;color:#8b95a3;text-align:center;margin:0 0 32px;line-height:1.6">
                You recently requested to ${actionText} for your Founders Hook account. Use the code below to verify it.
              </p>
