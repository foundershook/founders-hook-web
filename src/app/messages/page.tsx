...
function MessagesContent() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeConversations(
      ...
      (convos) => {
        ...
        // If redirected from notification with conversationId, select that conversation
        if (targetConversationId) {
          const matched = convos.find(
            (c: any) =>
              c._id === targetConversationId ||
              c.applicationId === targetConversationId ||
              c.application?._id === targetConversationId ||
              c._id === `app_${targetConversationId}` ||
              c._id === targetConversationId.replace(/^app_/, "") ||
              c.applicationId === targetConversationId.replace(/^app_/, "")
          );
          if (matched) {
    ...
    return () => unsubscribe();
  }, [me?.id, syncConversations, targetConversationId]);
  ...
  const handleEndMeet = async (messageId?: string) => {
              ...
              <>
                ...
                <div className="h-16 border-b border-ink-800/60 px-4 md:px-6 flex items-center justify-between shrink-0 bg-ink-900/20 backdrop-blur-md">
                  ...
                  <div className="flex items-center gap-2 sm:gap-3">
                    {/* Google Meet Button (Only founders can start a Google Meet) */}
                    {(me?.isFounder && !isApplicant) && (
                      <button
                        onClick={handleStartMeet}
                        disabled={startingMeet}
                        title="Start a Google Meet and invite candidate to video call"
                        className="flex items-center gap-1.5 px-3.5 py-1.5 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-ink-950 font-bold text-xs rounded-xl shadow-md shadow-emerald-500/20 transition-all transform active:scale-95 disabled:opacity-50 cursor-pointer"
                      >
                        {startingMeet ? (
                ...
                {(() => {
                  const chatMessages = messages
                    .filter((m) => m.type !== "application_card" && !m.applicationData)
                    .sort((a, b) => {
                      const tA = new Date(a.createdAt || 0).getTime();
                      const tB = new Date(b.createdAt || 0).getTime();
                      return (isNaN(tA) ? 0 : tA) - (isNaN(tB) ? 0 : tB);
                    });
                  const hasFounderMessaged = chatMessages.some(
                    (m) => String(m.sender?._id || "") !== String(me?.id || "")
                  );
                    ...
                    <>
                      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 custom-scrollbar">
                        {chatMessages.length === 0 ? (
                          <div className="flex-1 h-full min-h-[300px] flex flex-col items-center justify-center text-center p-8">
                            <div className="w-16 h-16 bg-ink-900/80 rounded-full flex items-center justify-center mb-3 border border-ink-800">
                              <MessageSquare size={24} className="text-sand-500" />
                            </div>
                            <p className="text-sm font-semibold text-sand-300">No messages yet</p>
                            <p className="text-xs text-sand-500 mt-1 max-w-xs">
                              {isApplicant
                                ? "Waiting for the startup founder to message you."
                                : "Send a message to start the conversation with the candidate."}
                            </p>
                          </div>
                        ) : (
                          chatMessages.map((msg, i) => {
                            ...
                            const showAvatar = !isMe && (i === 0 || chatMessages[i - 1]?.sender?._id !== msg.sender?._id);
                                    ...
                                    msg.meetStatus === "ended" || msg.content.includes("ended") ? (
                                      ...
                                      <div className="bg-gradient-to-br from-ink-900 via-ink-850 to-ink-900 border border-emerald-500/40 p-4 rounded-2xl shadow-xl shadow-emerald-950/40 max-w-xs sm:max-w-sm">
                                        ...
                                        <div className="flex flex-col gap-2">
                                          ...
                                          {isMe && (
                                            <button
                                              onClick={() => handleEndMeet(msg._id)}
                                              className="w-full flex items-center justify-center gap-1.5 py-1.5 px-3 bg-ink-800/80 hover:bg-rose-900/30 text-sand-400 hover:text-rose-400 border border-ink-700/60 hover:border-rose-700/40 font-medium text-[11px] rounded-lg transition-colors cursor-pointer"
                                            >
                                              <VideoOff size={12} />
                      ...
                      {/* Chat Input */}
                      {isApplicant && !hasFounderMessaged ? (
                        <div className="p-4 bg-ink-900/30 border-t border-ink-800/60 backdrop-blur-md text-center">
                          <p className="text-xs text-sand-400">
                            Only the startup founder can message the applicant. You will be able to reply once the founder reaches out.
                          </p>
                        </div>
                      ) : (
